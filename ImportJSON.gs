/*====================================================================================================================================*
  ...
  A library for importing JSON feeds into Google spreadsheets. Functions include:
     ...
     ImportJSONTokenAuth   For use by end users to import a JSON feed from a URL with HTTP Token Auth (added by Fábio Oliveira)
  ...
  1.6.1 (May 7, 2020) Added Token authentication with ImportJSONTokenAuth
...
/**
 * Helper function to authenticate with token auth information using ImportJSONAdvanced
 *
 * Imports a JSON feed and returns the results to be inserted into a Google Spreadsheet. The JSON feed is flattened to create
 * a two-dimensional array. The first row contains the headers, with each column header indicating the path to that data in
 * the JSON feed. The remaining rows contain the data.
...
function ImportJSONTokenAuth(url, token, query, parseOptions) {
  var header = {headers: {Authorization: "token " + token}};
  return ImportJSONAdvanced(url, header, query, parseOptions, includeXPath_, defaultTransform_);
}
